...
class _CamproleCreationState extends State<CamproleCreation> {
  ...
  Future<void> _fetchData() async {
    try {
      ...
      } else if (camps is Map && camps['data'] != null && camps['data'] is List) {
      ...
      } else if (getRoles is Map && getRoles['data'] != null && getRoles['data'] is List) {
  ...
  Future<void> _addAdminToDisaster(
    ...
    try {
      await TokenHttp().post('/auth/register', {
        ...
        'role': role == 'Camp Admin' ? 'campAdmin' :
                role == 'Collection Point Admin' ? 'collectionPointAdmin' :
                role == 'Statistics' ? 'stat' :
                role == 'Survey Officials' ? 'surveyOfficial' : 'verifyOfficial',
        'label': label,
        'assignPlace': role == 'Camp Admin' || role == 'Collection Point Admin'
            ? assignedTo : null,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Role Creation', style: TextStyle(color: Colors.white)),
        centerTitle: true,
        leading: IconButton(
                          ...
                          ElevatedButton(
                            onPressed: () {
                              if ((!nameController.text.isEmpty || _isEditingExistingUser) &&
                                ...
                                if ((role == 'Camp Admin' || role == 'Collection Point Admin') &&
                                    location.isEmpty) {
                                  setState(() {
                                ...
                                locationObject = campLocation.isNotEmpty
                                    ? campLocation
                                ...
                                child: ListTile(
                                  ...
                                  trailing: Row(
                                    ...
                                    children: [
                                      ...
                                      Wrap(
                                        spacing: 8.0,
                                        children: (admin['assignedRoles'] as List<dynamic>?)
                                            ?.map<Widget>((assignedRole) {
                                              return ElevatedButton(
                                                onPressed: () => deleteAdmins(
                                                  admin["_id"],
                                                  assignedRole,
                                                ),
                                                child: Row(
